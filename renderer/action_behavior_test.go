...
func TestUniversalValidateActionBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{"media item action", Universal{Form: &FormPage{Sections: []FormSection{{ID: "media", MediaItems: []MediaGalleryItem{{ID: "item", Actions: []Action{invalid}}}}}}}, `renderer.Universal: form section "media" media item "item" action "invalid": unsupported behavior "unsupported"`},
