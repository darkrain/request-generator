...
func TestRecordResourceResolutionAndPermissions(t *testing.T) {
	...
	parent := &BaseModule{Render: original, PageRenderFunc: func(_ *gin.Context, _ *renderer.Draft) error { return nil }}
	allowed, err := parent.RenderPageFor(c, renderer.PageTypeRecord)
	...
	denied, err := parent.RenderPageFor(c, renderer.PageTypeRecord)
	require.NoError(t, err)
